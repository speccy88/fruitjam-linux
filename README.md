...
| BusyBox userspace | `/bin`, `/usr/bin` | Supported | BusyBox tools plus `vi`, `hush`, web/network helpers, `free`/`uptime`/`fruitjam-mem`, tiny `ps`/`pgrep`/`pkill`, and Fruit Jam tools. | Keep applets constrained for no-MMU allocation behavior. |
...
| `fruitjam-pgrep`, `pgrep`, `pkill` | Tiny no-fork process lookup/signaling from `/proc` using substring or exact matching, avoiding `ps | grep | xargs` pipelines. |
...
| Tool inventory | PASS | `airliftctl`, `berry`, `fruitjamctl`, `fruitjam-i2c`, `fruitjam-rtttl`, `fruitjam-services`, `fruitjam-buttons`, `fruitjam-usbhost`, `fruitjam-hidkeys`, `mosquitto_pub`, `mosquitto_sub`, `fruitjam-mem`, `free`, `fruitjam-uptime`, `uptime`, `fruitjam-ps`, `ps`, `fruitjam-pgrep`, `pgrep`, `pkill`, `wget`, `telnet`, `nc`, `tftp`, and `vi` found in `PATH`. |
