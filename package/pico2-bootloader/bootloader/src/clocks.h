...
#define POWMAN_VREG_CTRL_REG			(0x04 / 4)
#define POWMAN_VREG_REG			(0x0c / 4)
#define POWMAN_PASSWORD_BITS			0x5afe0000u
#define POWMAN_VREG_CTRL_UNLOCK_BITS		BIT(13)
#define POWMAN_VREG_UPDATE_IN_PROGRESS_BITS	BIT(15)
#define POWMAN_VREG_VSEL_BITS			0x000001f0u
#define POWMAN_VREG_VSEL_LSB			4
#define POWMAN_VREG_VOLTAGE_1V30		0x0fu

#define MHZ		1000000UL

#define PLL_USB_HZ	(48UL * MHZ)
#define PLL_SYS_HZ	(252UL * MHZ)
...
#define CLK_HSTX	(CLK_SYS / 2)

static inline void set_vreg_1v30(void) {
	uint32_t vreg;

	POWMAN_BASE[POWMAN_VREG_CTRL_REG] |=
		POWMAN_PASSWORD_BITS | POWMAN_VREG_CTRL_UNLOCK_BITS;

	while ((POWMAN_BASE[POWMAN_VREG_REG] & POWMAN_VREG_UPDATE_IN_PROGRESS_BITS) != 0)
		;

	vreg = POWMAN_BASE[POWMAN_VREG_REG] & ~POWMAN_VREG_VSEL_BITS;
	vreg |= POWMAN_PASSWORD_BITS |
		(POWMAN_VREG_VOLTAGE_1V30 << POWMAN_VREG_VSEL_LSB);
	POWMAN_BASE[POWMAN_VREG_REG] = vreg;
...
static inline void set_hstx_clock_clk_sys(void) {
	...
	CLOCKS_BASE[CLOCKS_HSTX_CTRL_REG] |= BIT(CLOCKS_CTRL_ENABLE_SHIFT);
	CLOCKS_BASE[CLOCKS_HSTX_DIV_REG] = (CLK_SYS / CLK_HSTX) << CLOCKS_DIV_INT_SHIFT;
}
...
static inline void set_sys_pll(void) {
	...
	PLL_SYS_BASE[PLL_FBDIV_INT_REG] = 105; // 12MHz*105 = 1260MHz
	...
	// 1260MHz / (5 * 1) = 252MHz
	PLL_SYS_BASE[PLL_PRIM_REG] = (5 << PLL_PRIM_PDIV1_SHIFT) | (1 << PLL_PRIM_PDIV2_SHIFT);
